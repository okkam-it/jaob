...
public final class ReflectUtil {
  ...
  /**
   * Returns the IRI of the passed class.
   * 
   * @param clazz the Java class
   * @return the IRI of the passed class
   */
  ...
  private static IRI getIri(Class<?> clazz, OwlClass oc) {
    if (oc.uri().equals(Const.DEFAULT_ANNOTATION_STRING)) {
      if (clazz.getPackage().isAnnotationPresent(OwlOntology.class)) {
        return IRI.create(clazz.getPackage().getAnnotation(OwlOntology.class).uri() + "#"
            + clazz.getSimpleName());
      }
  ...
  /**
   * Return the base URI of the passed package.
   * 
   * @param p the java package
   * @return the base URI
   */
