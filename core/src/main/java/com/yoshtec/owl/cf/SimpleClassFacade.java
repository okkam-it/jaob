...
class SimpleClassFacade<T> extends BaseCF<T> implements ClassFacade {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(SimpleClassFacade.class);
  ...
  private void setRepresentedClass() throws IllegalStateException {
    ...
    for (Method method : clazz.getMethods()) {
      ...
      if (method.isAnnotationPresent(OwlDataProperty.class)) {
        LOG.warn("METHOD {} has unmanaged OwlDataProperty annotation", method.toGenericString());
      } else if (method.isAnnotationPresent(OwlObjectProperty.class)) {
        LOG.warn("METHOD {} has unmanaged OwlObjectProperty annotation", method.toGenericString());
  ...
  /**
   * Checks if the passed class is declared as an OwlClass and tries to retrieve the OwlClass URI.
   * 
    ...
    if (field.isAnnotationPresent(OwlDataProperty.class)) {
      ...
      if (cl.isArray()) {
        ...
        throw new IllegalStateException(
            "Arrays currently not supported in automatic Type detection");
  ...
  // returns SuperClasses of {@code c} including {@code c} itself, excluding
  // {@code java.lang.Object}. If {@code c} is {@code null} then empty.
   ...
   */
  public Object getNewInstance(String id) throws IllegalArgumentException, InstantiationException,
