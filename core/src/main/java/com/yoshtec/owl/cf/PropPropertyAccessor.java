...
public class PropPropertyAccessor implements PropertyAccessor {
  ...
  /**
   * Allows to do lazy property setting, it is mostly used to be able to cache Collections or arrays
  ...
  public PropPropertyAccessor(Method getter, Method setter, IRI propUri, XmlAdapter<?, ?> adapter,
      XsdTypeMapper typeMapper) {
    this.init(getter, setter, propUri, adapter, typeMapper);
  }

  private void init(Method getter, Method setter, IRI propUri, XmlAdapter<?, ?> adapter,
      XsdTypeMapper typeMapper) {
    ...
    if (getter.getDeclaringClass() != getter.getDeclaringClass()) {
      log.warn("Setter and getter defined in different classes {}, {}", getter.toGenericString(),
  ...
  /**
   * Extracts Datatype Annotations from from an {@link AnnotatedElement} and adds them to the dtUri
   * variable.
   * 
  ...
  /**
   * Returns the Getter if this Property is accessed via setter/getter pairs {@code null} otherwise.
   */
  ...
  /**
   * Returns the setter Method if this Property is accessed via setter/getter pairs {@code null}
   * otherwise.
   */
