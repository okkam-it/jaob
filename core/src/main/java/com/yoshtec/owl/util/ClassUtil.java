...
public final class ClassUtil {
	...
	/**
	 * Collects all the currently avialable classes in a 
	 * package. E.g. all the classes that are in the class path.
	 * 
	 * Taken from the 
	 * <a href="http://forum.java.sun.com/thread.jspa?threadID=341935&start=15&tstart=0">
	 * Java Development Forums</a>
	 * 
			...
			while (resources.hasMoreElements()) {
				...
				if (res.getProtocol().equalsIgnoreCase("jar")){
					...
					for (JarEntry e : Collections.list(jar.entries())){
						...
						if (e.getName().startsWith(pathpckgname) 
