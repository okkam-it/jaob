...
public class Codegen {
	...
	private OWLOntology loadOntology() throws OWLOntologyCreationException{
		...
		if(ontologyPhysicalIri != null && ontologyIri != null){
			// Create a physical IRI which can be resolved to point to where our ontology will be saved.
	...
	private void annotatePackage(){
		...
		if(this.ontologyPhysicalIri != null)
			jdoc.append("Loaded from IRI: <code>").append(ontologyPhysicalIri).append("</code>");
		...
		if(!imports.isEmpty()){
			...
			for(OWLImportsDeclaration imprt : imports ){
				jdoc.append("\t<li><code>").append(imprt.getIRI().toString()).append("</code></li>\n");
				annot.param(imprt.getIRI().toString());
			}
			jdoc.append("<ul>\n</br>\n</br>\n");
		}
	...
	private void addProperties(){
		...
		// Data Properties
		for(OWLProperty<?,?> dprop : ontology.getDataPropertiesInSignature(true)){
			this.addProperties(dprop, PropertyType.DATA);
		}
		
		// Object Properties
		for(OWLProperty<?,?> oprop : ontology.getObjectPropertiesInSignature(true)){
	...
	private void addProperty(OWLProperty<?,?> prop, JInterfaceProxy iface, PropertyType type){
		...
		StringBuffer pName = new StringBuffer(fragment.toString()); // TODO: better to use some visitor thing here i guess
...
//					String dturi = literal.getDatatype().getIRI().toString();
					IRI iri = oannot.getProperty().getIRI();//TODO ------------------
